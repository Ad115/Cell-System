...
class CellLine:
    ...
    def __init__(self,
        ...
        for letter in genome:
            if letter not in genomeAlphabet:
                raise ValueError('`genome` must contain only letters present \
                    in the `genomeAlphabet`')
    ...
    def newCell(self):
        ...
        if self.recycleDeadCells and self.deadCells:
            ...
            new = Cell(
                system=self.system, cellLine=self, index=self.currentIndex)
            self.currentIndex += 1
...
class Cell:
    ...
    Attributes
    ----------
            + Index: A label that identifies it among others in the same \
                     lineage.
    ...
    def divide(self, preserveFather=False):
        ...
        print("\tNew cells: {} @ {} and {} @ {}".format(
            self.index,
            self.getCoordinates(), daughter.index, daughter.getCoordinates()))

    # ---

    def divisionProbability(self):
        """This return the probability that this cell will divide if it was \
    ...
    def die(self):
        ...
        print("Cell no. {} dying @ site {} (father {})".format(
            self.index, self.getCoordinates(), self.father))
    ...
    def mutate(self):
        ...
        print("Mutating cell no. {} @ site {} (father {}):\n \
              ...
              \t Current mutations: {}"
              .format(self.index,
                      self.getCoordinates(), self.father,
                      self.getBaseGenome(), self.getGenome(), self.mutations))
        ...
        print("\t\t Final mutations: {}\n \
               \t Final genome: {}".format(self.mutations, self.getGenome()))
    ...
    def availableActions(self):
        ...
        """
        # Death action
        death = Cell._Action(self.die, self.deathProbability)
        # Division action
        division = Cell._Action(self.divide, self.divisionProbability)

        # Migration action
        migration = Cell._Action(self.migrate, self.migrationProbability)

        # Mutation action
        mutation = Cell._Action(self.mutate, self.mutationProbability)
